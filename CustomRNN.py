...
class CustomRNN(nn.Module):
    def __init__(self, cir_length, sym_blk, num_blk, device):
        super(CustomRNN,self).__init__()
        self.list = nn.ModuleList([RNNModule(cir_length, sym_blk).to(device) for i in range(0, num_blk)])
    ...
    def forward(self, input):
        x = torch.zeros((self.sym_blk*self.num_blk, self.cir_length*2),device=self.dev)
        tmp = self.list[0](input[0,:],torch.zeros((self.sym_blk*self.cir_length*2),device=self.dev)).to(self.dev)
